...
typedef struct {
  ...
  uint32_t hr_attr;   // Breath rate attribute
} chip_state_t;
...
void chip_timer_event(void *user_data) {
  ...
  // Read breath rate attribute and convert to voltage (assuming 0-30 range)
  float breathRate = attr_read_float(chip->hr_attr);
  float voltage = breathRate / 30.0 * 5.0;
...
void chip_init() {
  ...
  chip->hr_attr = attr_init("breathRate", 15); // Initialize breath rate attribute
  ...
  printf("Sensor de frecuencia respiratoria!\n");
}
